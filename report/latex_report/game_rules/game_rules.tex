...
Third, if the destination tile is controlled by the opponent, the move is only legal if the moving stack is strong enough to defeat the defending stack. This interaction is governed by the notion of power, described in the next section.
...
If the attacking power is strictly greater than twice the defending power, the move is a capture. In this case, the opponent’s pieces located on the destination tile are removed from the game. The attacking pieces then occupy the destination tile. An example of such a capture is shown in~\ref{fig:capture_rule}.
...
\begin{figure}[h]
    ...
    \caption{Example of a finished game won by the player with the black pieces}
    \label{fig:black_win}
\end{figure}
