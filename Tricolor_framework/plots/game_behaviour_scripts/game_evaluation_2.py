...
df = pd.read_csv("../../csv_files/game_behaviour_" + args[0] + "_vs_" + args[1] + "/game_evaluation_2.csv")

y = df["game_evaluation_2"]
x = np.arange(len(y))
...
plt.title("Game 2 Evaluation" + "\n" + args[0] + " vs " + args[1])

plt.tight_layout()
os.makedirs("../game_behaviour" + "_" + args[0] + "_vs_" + args[1], exist_ok=True)
plt.savefig("../game_behaviour" + "_" + args[0] + "_vs_" + args[1] + "/game_evaluation_2.pdf")